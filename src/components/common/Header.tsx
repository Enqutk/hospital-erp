...
export const Header: React.FC<HeaderProps> = ({
  ...
  const getModuleTitle = (tab: string) => {
    switch (tab) {
      case 'DASHBOARD': return 'Hospital Executive Dashboard & Telemetry';
      ...
      case 'IPD': return 'IPD Wards & Beds Allocation';
      case 'LAB_BLOOD':
          ...
          <div className="relative flex-1 max-w-md hidden md:block">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-12 py-1.5 text-xs bg-slate-50 hover:bg-slate-100/80 focus:bg-white border border-slate-200 focus:border-emerald-600 rounded-lg outline-hidden transition-all text-slate-900 placeholder:text-slate-400"
              />
              {patientSearchQuery && (
                <button
                  onClick={() => {
                ...
                {filteredPatients.length === 0 ? (
                  ...
                  filteredPatients.map((p) => (
                    <button
                      ...
                      className={`w-full px-3 py-1.5 text-left text-xs hover:bg-slate-50 flex items-center justify-between transition-colors cursor-pointer ${
                        selectedPatientMrn === p.mrn ? 'bg-emerald-50 text-emerald-950 font-medium' : ''
                      }`}
                    >
          ...
          <div className="flex items-center space-x-2">
            ...
            {onOpenNewPatientModal && (
              <button
                ...
                className="flex items-center gap-1.5 bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors cursor-pointer"
              >
            ...
            <div className="flex items-center border-l border-slate-200 pl-2 gap-1.5">
              <button
                ...
                className="flex items-center gap-2 p-1 hover:bg-slate-100 rounded-lg text-left transition-colors cursor-pointer"
                title="View Profile"
              >
                <img
